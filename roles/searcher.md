...
**On the built-in PDF reader.** Agent-harness PDF readers can silently truncate documents with malformed cross-reference tables and then *misreport the page count* (a 300-page book presenting as 17 pages of front matter). Read PDF *content* through the text layer; use the built-in reader only to view **individual pages as images** when taking the final verbatim text — and compare its reported page count against the PDF's actual `/Count` first.

**The page-position trap.** In works paginated at the **foot** of the page, an extracted text layer attributes pages **shifted by one, systematically**: the page number sits at the bottom and falls into the *next* page's text flow, so what the text layer presents as "page N" is printed page N−1. Header-style pagination does not have this problem. **Rule**: the text layer gives you the neighbourhood; the **page number you report is the one printed on the PDF page you actually opened**. When the two disagree, that is this trap — report the printed number and note "text layer off by one (footer pagination)", and treat every other page number cited from that same work as suspect until checked. This was found the hard way: a batch of "wrong page" audit failures turned out to be the extraction shifting, not the citations being wrong.
