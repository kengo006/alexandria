...
> 🔑 **And the probe's blind spot is structural, not a tuning problem: the probe reads the PDF, while the damage happens inside the extractor.** One file measured a perfectly healthy 3.42% single-letter rate at the source and came out of three extractors at fragment rates of **1.02% / 4.06% / 26.69%** — the worst of them splitting ordinary words into two- and three-letter pieces. Nothing about the file predicted that. ⇒ **The fix is not a cleverer pre-extraction probe. It is to measure the product every time**: choosing an extractor is not a one-time decision, it is a decision followed by a measurement, per file. Set a threshold, and when a file exceeds it, extract again with a different engine and re-measure.
>
> ⚠ **Choose that metric carefully — "fragment rate" is not "single-letter rate".** Counting every one-letter token punishes languages and authors who legitimately use them; in one work **43.6%** of single letters were legitimate. A metric that makes "books that use the word *a* a lot" look damaged will be quietly ignored, which is worse than not having it.
...
**The repair itself can be the new defect — and the safest-looking case is the dangerous one.** Before shipping any automated repair, ask the question that pairs with "what would this fire on in a healthy file?": **what does it look like when it is wrong?** If the wrong output is a *real word*, the repair must never be applied in bulk. A split fragment here joined into a perfectly ordinary English word, and the gate accepted it on exactly that ground — while the printed page carried a **surname with a dropped ligature**. Applying the rule would have replaced a person's name with a word that reads as completely normal, and **no downstream gate would ever light up again**. Same class: two fragments that are each a real word in another language; a fragment that is genuinely the tail of a legitimate phrase. The rule that follows is not "be careful" but a mechanism: a **deny list with dated evidence per entry**, a `--show` mode that prints surrounding context for human reading, and regression tests whose cases are the false positives you actually met. *The more natural a repair reads, the less likely anyone is to check it.*

**Intake runs everything; tiering decides how much a human reads, not what runs.** The reflex is to grade files by risk and run less on the safe ones. Measured here, file-level quality indicators had almost no discriminating power — the median damaged file and the median clean file were indistinguishable, and files that read perfectly well carried real defects. **Defects are not concentrated in bad-looking files, so any "low risk, run less" design leaks.** The asymmetry decides the default: doing the full work at intake is *O(one file)*; doing it afterwards is *O(the whole corpus)*. So **intake runs the full ladder by default, and whoever wants tiering to save work carries the burden of proof** — and that proof is a measurement of the indicator's predictive power, not an intuition. One indicator did earn its place: the PDF's **producer string**, readable before extraction. Files emitted by *re-packaging* tools were uniformly damaged; files emitted by native typesetters were uniformly clean — and the mechanism is known rather than merely correlated (re-packaging rewrites font and encoding tables, which is exactly where the character map is lost). Even so, it changes the *expectation*, not the workload. ⚠ And nothing here removes the structural cost: **the day a new failure type is named, every previously ingested file has to be swept again.**

**Searchability decides the severity; *where the defect came from* decides whether you touch it — and a book's own error, corrected, is a forged source.** The text layer is a **locating** layer: the verbatim authority always stays in the PDF, so the only question that sets priority is *will a word someone would actually search for still be findable*. But that question, on its own, rules "fix it" on typographic errors that are **printed in the book** — searching the correct spelling genuinely fails to find them. Three cases here were confirmed against the page image as the work's own misprints. Repairing those would mean anyone citing from the text layer produces a quotation **that does not exist in the source**. ⇒ Grade by searchability as before; then, before touching anything, ask **whether the extractor made this or the printer did**. The latter is registered, never repaired, and flagged in the file's own warning line where it matters. *(Same coin, other face, as the bulk-repair rule above: one candidate repair there would have turned a word the author really wrote into a different, more ordinary one.)*

**A page-anchor gate: the number of page markers must reconcile with the source's own page count.** Splitting a two-up scan produces one file at twice the page count of the original; a batch that extracts the split file and then files the *unsplit* original as its source leaves every downstream page reference pointing into a different document. ⚠ **Both numbers report success** — the extraction produced 156 pages and the file registry recorded a 78-page work — **they are simply not counts of the same object.** Reconcile them explicitly at intake; a gate that never compares the two will never notice.
