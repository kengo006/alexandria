...
- **Claims and evidence** (`governance/claims-and-evidence.md`) — the credential layer: every claim that affects credibility is bound to a trace that could only exist if the work was actually done. The rendered-page credential, credential tiering, the negative-conclusion rule, and the five ways a check dies.
