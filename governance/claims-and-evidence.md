...
**The credential's floor — the page itself can be wrong, and then the credential confirms the error.** Everything above assumes the text layer may lie and the page is ground truth. One work upstream removes that assumption: it is a *reflowed typesetting* of a scanned original — **image count: 0**, so the damaged words are real glyphs, set as type, with nothing left to re-OCR and no rendering setting that helps. On one page the printed text reads *"the deprivatized province of **inferiority**"* where the argument requires *interiority*; two lines above, the same page prints *"interior domain"* — **the page contradicts itself**, with no need to consult the original language. 🔑 **The defect is a real English word**, so every layer passes: the quotation corresponds, it is not second-hand, the entity attribution is right, the folio is there, and the sentence really is printed on that page. ⇒ **a ✅ carrying a full credential, on a string that is not the author's text.** ⚠ Be exact about what is wrong with it: the string *is* on that page, in that copy. What it is not is what the author wrote.

**The defence, for as long as this is the only copy you have, is a list.** (Replacing the copy is the other defence, and the better one — but it is a procurement problem, not one the pipeline can solve.) Register such works — the upstream registry marks them `verbatim_unsafe` — and refuse the ✅ **before it is issued**: location and page numbers stay usable, verbatim quotation does not.

⚠ **And do not repair the word before quoting** — but not for the reason that first suggests itself. A repaired `interiority` **is** the author's wording and **is** in every properly set edition; what it is not is *on the page you are citing*. 🔑 **So the two options fail in mirror image: quote it as printed and the words are not the author's; repair it and the words are not on the page. Either way the pairing of quotation and page credential is false**, and that pairing is the whole product. 🔑 **The general shape outlives the one volume: a check whose ground truth can itself be defective needs a register of the cases where it is — because by construction the check cannot find them.** This is the same structure as the degradation registry one layer down (`shared/degradation-registry.md`), and for the same reason: what a gate cannot see has to be written down by someone who saw it.
...
**The line you added in order to be careful is where the blind spot lives.** A corpus census restricted itself to works of 120 printed pages or more — a defensible line, drawn so that double-column journal articles could not inflate a words-per-page statistic. But the condition being hunted *halves a work's page count*: books scanned two printed pages to a sheet. The single most affected work in the corpus — 212 printed pages on 106 sheets — fell under the threshold and never entered the census at all. It surfaced weeks later by accident, during unrelated reading. ⇒ **Every filter added to suppress false positives owes the mirror question: is it independent of the thing being detected?** 🔑 The first question — *will this let noise in?* — always gets asked. The second — *will this keep the real cases out?* — is the one that decides, and it is asked less often precisely because the filter was written in a spirit of caution, and caution does not feel like a source of error.
