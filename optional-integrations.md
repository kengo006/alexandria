...
- **The index self-reports its degraded set — every category of it**: the info endpoint reads the degradation registry (`shared/degradation-registry.md` — a living file; read fresh, never cached) and returns `degraded: {status, generated, counts: {<category>: n, ...}, meaning: {<category>: "one line of instruction", ...}}`. **Unreadable registry → `status: "unknown"` with no counts — never `0`**: "zero degraded files" asserts negative conclusions are safe everywhere; "couldn't read the registry" asserts nothing. If a category's own count disagrees with its row count, report the measured value and surface the discrepancy — don't silently pick one.
  ⚠ **Reporting one category's count is the failure mode to design against**, not a smaller version of doing it right: the registry's categories prescribe *opposite* actions (a scrambled-order file is fully grep-reliable), so a single `degraded_files` number leaves the agent confidently wrong about every category it omitted. Observed upstream: a file entered the reading-order category and the endpoint's output did not change at all.
